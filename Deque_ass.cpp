...
#define Deque_DEFINE(t)											\
	...
	void Deque_##t##_push_back(Deque_##t *deq,t ob){						\
		...
		deq->data[deq->back_pointer] = ob;							\
		strcpy(deq->data[deq->back_pointer++].name, ob.name);					\
		deq->Size++;										\
	}												\
	void Deque_##t##_push_front(Deque_##t *deq, t ob){						\
		if(deq->front_pointer == 0 || deq->front_pointer-1 == deq->back_pointer)		\
			...
			deq->front_pointer--;								\
		deq->data[deq->front_pointer].id = ob.id;						\
		strcpy(deq->data[deq->front_pointer].name, ob.name);					\
		deq->Size++;										\
	}												\
	...
	bool Deque_##t##_equal(Deque_##t deq1, Deque_##t deq2){						\
		if(deq1.Size == deq2.Size) {								\
			...
			for(i=0;i<deq1.Size;i++){							\
				if(!(deq1.at(&deq1, i).id == deq2.at(&deq2, i).id && !(strcmp(deq1.at(&deq1, i).name,deq2.at(&deq2, i).name))))\
					return 0;							\
			}										\
			return 1;									\
													\
...
int main() {
	...
	/*deq.d.id = 100;
	strcpy(deq.d.name, "Anuroop");
	cout<<"At the back: "<<deq.data[0].id<<" "<<deq.data[0].name<<endl;
	cout<<"At the back: "<<deq.data[1].id<<" "<<deq.data[1].name<<endl;
	cout<<"At the back: "<<deq.data[2].id<<" "<<deq.data[2].name<<endl;
	cout<<"At the back: "<<deq.data[3].id<<" "<<deq.data[3].name<<endl;
	cout<<"At the back: "<<deq.data[4].id<<" "<<deq.data[4].name<<endl;
	MyClass_print(&deq.d);*/
//	MyClass_print(&deq.front(&deq));
//	MyClass_print(&deq.back(&deq));
	//cout<<"At the front is: "<< deq.front(&deq).id<<endl;
